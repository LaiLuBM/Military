				...
				<el-card>
					...
					>
						...
						<el-table-column
							align="center"
							label="最近执行预测时间"
							prop="equipMalfDetail.predictTime"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipMalfDetail?.predictTime || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近预测算法"
							prop="equipMalfDetail.modelAlgorithm"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipMalfDetail?.modelAlgorithm || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近预测故障类型"
							prop="equipMalfDetail.failureType"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipMalfDetail?.failureType || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近预测故障最可能月份"
							width="103px"
							prop="equipMalfDetail.failureTiming"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipMalfDetail?.failureTiming || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="月份对应故障概率"
							prop="equipMalfDetail.failureProbability"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{
									formatPercentage(
										scope.row?.equipMalfDetail?.failureProbability,
									)
								}}
							</template>
						</el-table-column>
...
import { predictApi, predictstartApi } from "@/api/fault/predict";
...
const saveSearchParams = () => {
	safeSessionStorage.set(
		"finingFaultPredictSearchParams",
		JSON.stringify({
			...searchParams.value,
...
const restoreSearchParams = () => {
	try {
		const saved = safeSessionStorage.get("finingFaultPredictSearchParams");
		if (saved) {
...
const handlePredict = async () => {
	...
	try {
		...
		if (response.code === 200) {
			// Map response data to create updatedEquipments with predictId
			const updatedEquipments = response.data.map((prediction: any) => {
				...
				return {
					...
					equipSimpleName: equip
						...
						: "未知型号",
					algorithm: equip ? equip.algorithm || "LSTM" : "LSTM",
			...
			if (response.data.length > 0) {
				...
				router.push(
					`/predict/finingdetail?equipCode=${firstEquip.equipCode}&predictId=${firstEquip.predictId}&sessionId=${sessionId}`,
				);
			}
		} else {
...
const handleDetail = (equipCode: string) => {
	...
	const predictId = row?.equipMalfDetail?.predictId || null;
	if (row.equipMalfDetail?.predictTime) {
		router.push(
