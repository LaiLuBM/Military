      ...
      <el-card class="mt"> -->
					...
					>
						...
						<el-table-column
							...
							label="近三个月预测次数"
							width="112px"
							prop="predictRepairTimes"
							sortable
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.predictRepairTimes || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近执行预测时间"
							prop="equipRepair.predictTime"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipRepair?.predictTime || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近预测算法"
							prop="equipRepair.modelAlgorithm"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipRepair?.modelAlgorithm || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近送修类型"
							prop="equipRepair.repairType"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{ scope.row?.equipRepair?.repairType || "无" }}
							</template>
						</el-table-column>
						<el-table-column
							align="center"
							label="最近送修可能时间"
							prop="equipRepair.repairTime"
						 :show-overflow-tooltip="true">
							<template #default="scope">
								{{
									formatRepairTime(scope.row?.equipRepair?.repairTime) || "无"
								}}
								<!-- {{ scope.row?.equipRepair?.repairTime || '无' }} -->
							</template>
						</el-table-column>
									...
									<el-tooltip raw-content placement="top" effect="light">
										<template #content>
											<span style="text-align: right"
												...
												RBF：径向基函数神经网络，拟合能力强，精度高，适合大数据；<br />
...
import { predictrepairApi } from "@/api/fault/predict-repair";
...
const restoreSearchParams = () => {
	try {
		const saved = safeSessionStorage.get("generalTimeSearchParams");
		if (saved) {
...
const handleEquipInfo = (equipCode: string) => {
	// addTab("基本详细信息", "/deviceinformation/Detailinformation", "Warning");
	// setCurrentTab("基本详细信息", "/deviceinformation/Detailinformation");
...
const handlePredict = async () => {
	...
	const modelAlgorithms = selectedEquipments.value.map(
		(item) => item.algorithm || "DecisionTree",
	);
	const userId = "12354";
	const isDetailed = false;

	try {
		const response = await predictrepairApi(
			equipCodes,
			isDetailed,
			modelAlgorithms,
			sessionId,
		...
		if (response.code === 200) {
			safeSessionStorage.set("selectedEquipments", "");
			safeSessionStorage.set("selectedEquipCodes", "");
			const updatedEquipments = response.data.map((prediction: any) => {
				...
				return {
					...
					equipSimpleName: equip
						...
						: "未知型号",
					algorithm: equip ? equip.algorithm || "DecisionTree" : "DecisionTree",
					repairId: prediction.repairId,
					predictId: prediction.predictId, // Ensure compatibility if needed
				};
			});

			// Store in sessionStorage with consistent keys
			safeSessionStorage.set(
				"selectedEquipments",
			...
			// Navigate to Repairdetail with the first equipment's equipCode and repairId
			if (response.data.length > 0) {
				const firstEquip = response.data[0];
				router.push(
					`/predict/repairdetail?equipCode=${firstEquip.equipCode}&repairId=${firstEquip.repairId}&sessionId=${sessionId}`,
				);
			}
		} else {
			ElMessage.error("预测执行失败: " + response.message);
			isPredictButtonDisabled.value = false;
		}
	} catch (error) {
		console.error("预测执行失败:", error);
		ElMessage.error("预测执行失败，请稍后重试");
		isPredictButtonDisabled.value = false;
	}
...
const handleDetail = (equipCode: string) => {
	...
	const repairId = row?.equipRepair?.repairId || null;

	// 同时检查 predictTime 和 repairId
	if (row?.equipRepair?.predictTime && repairId) {
		console.log("准备跳转到repairdetail页面");
		router.push(
			`/predict/repairdetail?equipCode=${equipCode}&repairId=${repairId}`,
		);
	} else {
		ElMessage.warning("该装备暂无有效的预测记录");
	}
};
...
const handleHistory = (
	...
	sessionStorage.setItem("repairHistoryEquipCode", equipCode);
	sessionStorage.setItem("repairHistoryEquipName", equipSimpleName);
	sessionStorage.setItem("repairHistoryUnitName", unitSimpleName);

	// addTab('送修预测结果历史记录', '/predict/repair_history', 'Warning');
	// setCurrentTab('送修预测结果历史记录', '/predict/repair_history');

	const row = dataList.value.find((item: any) => item.equipCode === equipCode);
	if (row.equipRepair?.predictTime) {
		router.push({
			path: "/predict/repairhistory",
			query: { equipCode, equipSimpleName, unitSimpleName: selectedUnit.value },
		});
	} else {
		return ElMessage.warning("该装备暂无历史预测记录");
	}
};
