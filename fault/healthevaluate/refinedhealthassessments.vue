		...
		<el-card>
			...
			<el-row>
				...
				>
					...
					<el-table-column
						label="分系统健康指数"
						prop="subsysNameAndScore"
						align="center"
					 :show-overflow-tooltip="true">
						<template #default="{ row }">
							{{ formatSubsysNameAndScore(row.subsysNameAndScore) || "无" }}
						</template>
					</el-table-column>
					<el-table-column
						label="核准状态"
					...
					<el-table-column prop="algorithm" width="120" align="center" :show-overflow-tooltip="true">
						<template #header>
							<span
								style="
									display: flex;
					...
					<el-table-column label="操作" align="center" width="280" :show-overflow-tooltip="true">
						<template #default="scope">
...
import { equipmodelhealth } from "@/api/fault/finingequipmodel";
...
const formatSubsysNameAndScore = (subsysNameAndScore: string) => {
	if (!subsysNameAndScore) return "";
	return subsysNameAndScore
		.replace(/[{}]/g, "")
		.split(", ")
		.map((item) => item.trim())
		.join(" ");
...
const restoreSearchParams = async() => {
	try {
		const saved = safeSessionStorage.get("refinedHealthSearchParams");
		if (saved) {
...
// 	try {
// 		const response = await equipmodelhealth(unitPath);
...
//    try {
//       const response = await equipmodelhealth(nodeData.unitPath);
//       console.log("装备型号数据:", response);
...
const handleHistory = (equipCode: string, approvalStatus: string) => {
	router.push({
		path: "/healthevaluate/fininghealthhistory",
	...
	try {
		...
		if (unitCode) {
			...
			if (unit) {
				...
				console.warn(`Unit with unitCode ${unitCode} not found`);
				ElMessage.warning(`未找到单位编码 ${unitCode} 对应的单位名称`);
			}

			// Fetch equipment models for the selected unit
			if (unit?.unitPath) {
				const response = await equipmodelhealth(unit.unitPath);
				if (response.code === 200 && response.data?.categories) {
					const groupedOptions = Object.entries(response.data.categories).map(
						([category, items]) => {
							if (!Array.isArray(items)) {
					...
					if (equipModelCode) {
						...
						if (foundOption) {
							...
							console.warn(`Equipment model ${equipModelCode} not found`);
							ElMessage.warning(
...
// // 添加 activated 钩子
// onActivated(() => {
// 	console.log("refinedhealthassessments.vue activated");
