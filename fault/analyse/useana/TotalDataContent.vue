...
import { malfregioncount } from '@/api/fault/strengthanalyse';
...
const renderChart = (data: any) => {
    ...
    let chartTitle = '故障预测数据总量';
    let malfCountData: number[] = new Array(fixedRanges.length).fill(0);
    let countData: number[] = new Array(fixedRanges.length).fill(0);
    chartDescription.value = '暂无数据，请选择单位/分队和装备型号后查询';

    if (data && data.malfCountChart) {
        const { chartTitle: title, malfCountChart, description } = data;
        const { equipMotorRange, equipMalfCount, equipCount } = malfCountChart;

        chartTitle = title || chartTitle;
        chartDescription.value = description || '';

        // 处理后端数据
        const rangesArray = typeof equipMotorRange === 'string' ? equipMotorRange.split(',').map(r => r.trim()) : equipMotorRange || [];
        ...
        console.log('malfCountArray:', malfCountArray);
        console.log('countArray:', countArray);

        // 映射数据到固定范围
        rangesArray.forEach((range: string, index: number) => {
            const fixedIndex = fixedRanges.indexOf(range);
            if (fixedIndex !== -1 && index < malfCountArray.length && index < countArray.length) {
                malfCountData[fixedIndex] = malfCountArray[index] || 0;
                countData[fixedIndex] = countArray[index] || 0;
            } else if (fixedIndex === -1) {
                console.warn(`Range ${range} not found in fixedRanges`);
            }
        });

        // 调试日志：映射后的数据
        console.log('malfCountData:', malfCountData);
