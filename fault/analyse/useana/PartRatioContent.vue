...
import { malfregionsubpart } from '@/api/fault/strengthanalyse';
...
const renderChart = async (data: any) => {
    ...
    if (!data || !data.malfRatioChart || !data.malfCountChartList) {
        ...
        // Default Left Chart (Pie Chart)
        const defaultParts = ['发动机', '液压器', '操纵装置'];
        const pieData = defaultParts.map(part => ({
            name: part,
            value: 1, // Small non-zero value for equal slices
        }));

        const pieOption = {
            color: defaultParts.map(part => colors[part] || '#999'),
            title: {
                text: '部件故障占比',
                left: 'center',
                top: 10,
            ...
            series: [
                {
                    ...
                    radius: '45%',
                    center: ['50%', '50%'],
                    data: pieData, // Use the mapped pieData
                    emphasis: {
                        itemStyle: {
                            shadowBlur: 10,
                            shadowOffsetX: 0,
        ...
        // Default Right Chart (Line Chart)
        const defaultMotorRanges = ['0-100', '100-200', '200-300', '300-400'];
        const lineSeries = defaultParts.map(part => ({
            name: part,
            type: 'line',
            data: [0, 0, 0, 0],
            smooth: true,
            symbol: 'circle',
            symbolSize: 8,
            lineStyle: { color: colors[part] || '#999' },
            itemStyle: { color: colors[part] || '#999' },
        }));
        ...
        const lineOption = {
            ...
            yAxis: {
                ...
                name: '故障数',
                min: 0,
                max: 10, // Arbitrary max for default chart
            },
            series: lineSeries,
        };
    ...
    const pieData = parts
        .map((part: string, index: number) => ({
            name: part, // Corrected from Verona
            value: counts[index],
        }))
        .filter((item: { value: number }) => item.value > 0); // 过滤掉计数为 0 的部件

    const pieOption = {
        color: pieData.map((item: any) => colors[item.name] || '#999'),
        title: {
            text: malfRatioChart.chartTitle || '部件故障占比',
            left: 'center',
            top: 10,
            textStyle: {
                fontSize: 16,
    ...
    const lineSeries = malfCountChartList.map((item: any) => {
        ...
        return {
            ...
            symbol: 'circle',
            symbolSize: 8,
            lineStyle: { color: colors[item.equipMalfPart] || '#999' },
            itemStyle: { color: colors[item.equipMalfPart] || '#999' },
        };
    });

    const lineOption = {
        color: malfCountChartList.map((item: any) => colors[item.equipMalfPart] || '#999'),
        title: {
            text: chartTitle || '部件故障数趋势',
            left: 'center',
            top: 10,
            textStyle: {
                fontSize: 16,
