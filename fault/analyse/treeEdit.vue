        ...
        <el-row :gutter="20">
            <el-col :span="1">
                <el-button text @click="goBack" icon="ArrowLeft"
                    style="font-size: 30px; padding: 0;margin: 10px 20px;" />
            </el-col>
            ...
            <el-col :span="12">
                ...
                <el-row>
                    <el-col :span="16">
                        <el-form ref="formRef" :model="formData" :rules="rules" label-width="120px"
                            ...
                            <el-form-item label="节点类型:" class="border-item">
                                <el-input v-model="formData.nodeType" placeholder="请输入节点类型" clearable readonly />
                            </el-form-item>
                    ...
                    <el-col :span="6" style="margin-left: 10px;">
                        ...
                        <!-- 其他状态显示当前状态 -->
                        <el-button v-else type="info" style="margin-top: 10px;" disabled>
                            {{ currentStatus || '加载中...' }}
                        </el-button>
                    </el-col>
                </el-row>
...
import { treeDataApi, treeUpdateApi, nodeUpdateApi, nodeDeleteApi, nodeAddApi } from '@/api/fault/faultTree';
...
const treeId = ref<string | undefined>(undefined);
const treeName = ref<string | undefined>(undefined);

// 树形结构属性配置
const treeProps = {
    label: (node: TreeNode) => `${node.nodeName}(${node.nodeType})`,
    children: 'children'
};
...
// 获取父节点名称
const getParentNodeName = (nodeId: number | null | undefined, nodes: TreeNode[]): string => {
...
const fetchTreeData = async () => {
    if (!treeId.value || !treeName.value) {
        ElMessage.warning('未提供 treeId 或 treeName，新建模式');
        treeData.value = [];
    ...
    try {
        ...
        if (response.code === 200) {
            ...
            // 关键：把状态也存下来
            currentStatus.value = faultTree.status || '未提交';
        ...
        const failed = Object.keys(error.fields);
        if (failed.includes('treeName')) {
            ElMessage.error('故障树名称不能为空或全是空格');
        } else if (failed.includes('creatorName')) {
            ElMessage.error('创建人不能为空或全是空格');
        } else {
            ElMessage.error('请正确填写必填信息');
        }
        return;
    }
};
...
const save = async () => {
    ...
    // 判断故障树级别字段是否修改（包括 description/solution 也属于树级别）
    const treeFieldsChanged = [
    ...
    // 情况1：修改了节点字段 → 更新节点 + 刷新树
    if (nodeId && nodeFieldsChanged) {
        await updateNode();        // 你原来的节点更新逻辑
        return;
    }

    // 情况2：修改了故障树字段 或 什么都没改 → 统一走 treeUpdateApi（最安全）
    // 重点：什么都没改也走这一支！后端会返回成功，不会报错
    if (treeId.value) {
        const editload = {
            creatorName: formData.value.creatorName.trim(),
            description: formData.value.description,
            solution: formData.value.solution,
            treeName: formData.value.treeName.trim(),
            treeId: formData.value.treeId,
        };
        ...
        try {
            ...
            if (response.code === 200) {
                ...
                // 无论改没改，都写 session，让列表页能自动带上最新 treeName
                safeSessionStorage.set('dataUpdated', 'true');
                safeSessionStorage.set('faultTreeData', JSON.stringify({
                    treeId: formData.value.treeId,
                    treeName: formData.value.treeName,
            ...
            ElMessage.error('保存请求出错');
        }
    } else {
        ElMessage.warning('缺少必要的树ID，无法保存');
    }
};
...
// 在组件挂载时获取数据
onMounted(() => {
    if (route.query.treeId && route.query.treeName) {
        treeId.value = route.query.treeId as string;
        treeName.value = route.query.treeName as string;
        fetchTreeData();
    }
