...
import { malfunitcount } from '@/api/fault/unitanalyse';
...
const renderChart = (data: any) => {
  ...
  const { equipMalfRegionUnitName, equipMalfCount, equipCount } = malfCountChart;

  // Parse comma-separated strings and ensure data integrity
  const unitNames = equipMalfRegionUnitName ? equipMalfRegionUnitName.split(',').map((name: string) => name.trim()) : [];
  const malfCounts = equipMalfCount
    ? equipMalfCount.split(',').map((count: string) => parseInt(count.trim()) || 0)
    : [];
  const equipCounts = equipCount
    ? equipCount.split(',').map((count: string) => parseInt(count.trim()) || 0)
    : [];
    ...
    series: [
      {
        ...
        barGap: '10%',
        itemStyle: {
          color: '#91CC75', // Blue for malfunction count
        },
        label: {
          show: false,
      ...
      },
      {
        ...
        barGap: '10%',
        itemStyle: {
          color: '#5470C6', // Green for equipment count
        },
        label: {
          show: false,
          position: 'insideRight',
...
// Handle node click for "隶属单位" dropdown
...
const handleQuery = async () => {
  const params = {
    ...
    unitList: selectedUnitPaths.value.join(','), // Use selectedUnitPaths instead of region
  };
  ...
  const response = await malfunitcount(params);
