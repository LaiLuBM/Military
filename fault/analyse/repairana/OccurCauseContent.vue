...
import { repairreason } from '@/api/fault/repairanalyse';
...
// ... (other refs and logic remain unchanged)
...
const renderChart = async (data: any) => {
  ...
  const pieReasons: string[] = (malfRatioChart.equipMalfReason || '').split(',').map((reason: string) => reason.trim()).filter(Boolean);
  ...
  const pieData = pieReasons.map((reason: string, index: number) => ({
    name: reason,
  ...
  // Generate random colors based on the number of unique reasons
  const uniqueReasons = [...new Set([...pieReasons, ...malfCountChartList.map((item: { equipMalfReason: string }) => item.equipMalfReason)])];
  const randomColors = generateRandomColors(uniqueReasons.length);
  const colorMap: Record<string, string> = uniqueReasons.reduce((acc, reason, index) => {
    acc[reason] = randomColors[index];
    return acc;
  }, {} as Record<string, string>);

  // Create line series with dynamic colors
  const lineSeries = malfCountChartList.map((item: { equipMalfReason: string; equipMalfCount: string }) => {
    ...
    return {
      ...
      symbolSize: 8,
      itemStyle: {
        color: colorMap[item.equipMalfReason] || '#999',
      },
    };
  });

  // Check if all data is zero
  const pieAllZero = pieData.every((d: { value: number }) => d.value === 0) || pieReasons.length === 0;
  ...
  // Pie Chart (Left - Fault Reason Distribution)
  const pieOption = {
    title: {
      text: pieAllZero ? `无数据 - ${malfRatioChart.chartTitle || '故障发生原因占比'}` : malfRatioChart.chartTitle || '故障发生原因占比',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 18,
  ...
  const lineOption = {
    title: {
      text: lineAllZero ? `无数据 - ${chartTitle || '故障原因按修理状态统计'}` : chartTitle || '故障原因按修理状态统计',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 18,
    ...
    legend: {
      data: malfCountChartList.map((item: { equipMalfReason: string }) => item.equipMalfReason),
      top: 70,
      left: 'center',
      textStyle: { fontSize: 12 },
    },
...
const handleQuery = async () => {
  ...
  const response = await repairreason(params);
