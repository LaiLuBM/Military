...
import { repaircount } from '@/api/fault/repairanalyse';
...
const renderChart = (data: any) => {
  ...
  const { equipMalfType, equipMalfCount, equipCount } = malfCountChart;

  // Update chart description
  chartDescription.value = description || '';

  // Split equipMalfType by comma if it’s a string, otherwise use as array
  const faultTypesArray: string[] = typeof equipMalfType === 'string' ? equipMalfType.split(',').map((type: string) => type.trim()) : (equipMalfType || []);
  const uniqueFaultTypes: string[] = [...new Set(faultTypesArray)].filter(type => type); // Remove duplicates and empty strings

  // Split equipMalfCount and equipCount by comma if they’re strings, otherwise use as arrays
  const malfCountArray: number[] = typeof equipMalfCount === 'string' ? equipMalfCount.split(',').map((count: string) => parseInt(count.trim()) || 0) : (equipMalfCount || []);
  const countArray: number[] = typeof equipCount === 'string' ? equipCount.split(',').map((count: string) => parseInt(count.trim()) || 0) : (equipCount || []);

  // Map counts to unique fault types
  const malfCountData: number[] = uniqueFaultTypes.map((type: string) => {
    const index = faultTypesArray.indexOf(type);
    return index !== -1 && index < malfCountArray.length ? malfCountArray[index] : 0;
  });
  const countData: number[] = uniqueFaultTypes.map((type: string) => {
    const index = faultTypesArray.indexOf(type);
    return index !== -1 && index < countArray.length ? countArray[index] : 0;
  });

  // Check if all data is zero
  const allZero = malfCountData.every((d: number) => d === 0) && countData.every((d: number) => d === 0);

  // Configure chart options
  chart.setOption({
    title: {
      text: allZero ? `无数据 - ${chartTitle || '故障预测数据总量'}` : chartTitle || '故障预测数据总量',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 20,
    ...
    legend: {
      data: ['装备总数', '故障数量'], // Order: Fault Count (left), Equipment Total (right)
      top: 70,
      left: 'center',
    ...
    series: [
      {
        ...
        barGap: '10%',
        itemStyle: {
          color: '#91cc75', // Blue for fault count
        },
      },
      {
        name: '装备总数',
        ...
        barGap: '10%',
        itemStyle: {
          color: '#5470c6', // Green for equipment total
        },
      },
    ],
    textStyle: { color: '#666' },
...
const handleQuery = async () => {
  ...
  const response = await repaircount(params);
