<template>
  <el-card>
    ...
    <!-- Add description below the chart, excluding '。' -->
...
import { repairsubsys } from '@/api/fault/repairanalyse';
...
const renderChart = async (data: any) => {
  ...
  const radarSubsys: string[] = (malfRatioChart?.equipMalfSubsys || '').split(',').map((subsys: string) => subsys.trim()).filter(Boolean);
  const radarCounts: number[] = (malfRatioChart?.equipMalfCount || '').split(',').map((count: string) => {
    const num = parseFloat(count.trim());
    return isNaN(num) || num < 0 || num > 1000 ? 0 : num;
  });
  const maxCount = Math.max(...radarCounts, 1);
  const radarValues = radarCounts.map(count => count / maxCount);
  const radarIndicators = radarSubsys.length > 0 ? radarSubsys.map((subsys: string) => ({
    name: subsys,
    max: 1,
  })) : [{ name: '无数据', max: 1 }];
  ...
  // Generate random colors based on the number of unique subsystems
  const uniqueSubsys = [...new Set([...radarSubsys, ...(malfCountChartList || []).map((item: { equipMalfSubsys: string }) => item.equipMalfSubsys)])];
  const randomColors = generateRandomColors(uniqueSubsys.length);
  const colorMap: Record<string, string> = uniqueSubsys.reduce((acc, subsys, index) => {
    acc[subsys] = randomColors[index];
    return acc;
  }, {} as Record<string, string>);

  // Create line series with dynamic colors
  const lineSeries = (malfCountChartList || []).map((item: { equipMalfSubsys: string; equipMalfCount: string }) => {
    ...
    return {
      ...
      symbolSize: 8,
      itemStyle: {
        color: colorMap[item.equipMalfSubsys] || '#999',
      },
    };
  });

  // Check if all data is zero
  const radarAllZero = radarValues.every((v: number) => v === 0) || radarSubsys.length === 0;
  const lineAllZero = lineSeries.every((s: { data: number[] }) => s.data.every((d: number) => d === 0)) || repairStatuses.length === 0;

  // Calculate y-axis max for line chart
  const allCounts = lineSeries.flatMap((series: { data: number[] }) => series.data).filter((num: number) => num > 0 && num < 1000);
  const yAxisMax = allCounts.length ? Math.ceil(Math.max(...allCounts) * 1.2) : 10;

  // Radar Chart (Left - Fault Subsystem Distribution)
  const radarOption = {
    title: {
      text: radarAllZero ? `无数据 - ${malfRatioChart?.chartTitle || '故障发生分系统占比'}` : malfRatioChart?.chartTitle || '故障发生分系统占比',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 18,
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        const percentage = Math.round((radarValues[index] * 100)) + '%';
        return `${radarSubsys[index] || '未知'}: ${percentage}`;
      },
    },
    legend: {
      orient: 'horizontal',
    ...
    radar: {
      ...
      splitArea: { areaStyle: { color: ['#f5f5f5', '#ffffff'] } },
    },
    series: [
      {
        name: '故障概率分布',
        ...
        data: radarAllZero ? [] : [{
          ...
          lineStyle: { width: 2 },
          itemStyle: {
            color: colorMap[radarSubsys[0]] || '#999',
          },
        }],
      },
    ],
  ...
  const lineOption = {
    title: {
      text: lineAllZero ? `无数据 - ${chartTitle || '分系统故障数统计'}` : chartTitle || '分系统故障数统计',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 18,
    ...
    legend: {
      data: (malfCountChartList || []).map((item: { equipMalfSubsys: string }) => item.equipMalfSubsys),
      top: 70,
      left: 'center',
      textStyle: { fontSize: 12 },
    },
  ...
  try {
    ...
    leftChart.setOption({
      title: {
        ...
        left: 'center',
        top: 'center',
        textStyle: { fontSize: 24, fontWeight: 'bold', color: '#999' },
      },
    });
...
onMounted(async () => {
  ...
  treeData.value = data;
  // Remove initial chart rendering to ensure it only happens after query
...
const handleQuery = async () => {
  ...
  const response = await repairsubsys(params);
