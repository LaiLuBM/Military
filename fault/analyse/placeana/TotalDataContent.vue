...
import { malfregionlist, malfregioncount } from '@/api/fault/placeanalyse';
...
const renderChart = (data: any) => {
  ...
  if (data && data.malfCountChart) {
    const { chartTitle, malfCountChart, description } = data;
    const { equipMalfRegion, equipMalfCount, equipCount } = malfCountChart;

    // Update the description
    chartDescription.value = description || '';

    // Split equipMalfRegion by comma if it’s a string, otherwise use as array
    const regionsArray = typeof equipMalfRegion === 'string' ? equipMalfRegion.split(',') : equipMalfRegion;
    const uniqueRegions = [...new Set(regionsArray)]; // Remove duplicates
    console.log('Unique regions:', uniqueRegions);

    // Split equipMalfCount and equipCount by comma if they’re strings, otherwise use as arrays
    const malfCountArray = typeof equipMalfCount === 'string' ? equipMalfCount.split(',').map(Number) : equipMalfCount;
    const countArray = typeof equipCount === 'string' ? equipCount.split(',').map(Number) : equipCount;

    // Map counts to unique regions
    const malfCountData = uniqueRegions.map((region) => {
      const index = regionsArray.indexOf(region);
      return index !== -1 && index < malfCountArray.length ? malfCountArray[index] || 0 : 0;
    });
    const countData = uniqueRegions.map((region) => {
      const index = regionsArray.indexOf(region);
      return index !== -1 && index < countArray.length ? countArray[index] || 0 : 0;
    });

    chart.setOption({
      title: {
        text: chartTitle || '故障预测数据总量',
        left: 'center',
        top: 10,
        textStyle: {
          fontSize: 24,
      ...
      xAxis: {
        type: 'value',
        min: 0, // Ensure starting point is an integer
        max: Math.max(...malfCountData, ...countData, 10), // Ensure max is sufficient for data, at least 10
        interval: 1, // Force intervals to be 1 (no decimals)
        splitNumber: Math.ceil(Math.max(...malfCountData, ...countData) / 1), // Ensure whole number intervals
        axisLabel: {
          formatter: '{value}' // Ensure labels are integers
        }
      },
      yAxis: { type: 'category', data: uniqueRegions },
      series: [
        { name: '故障数量', type: 'bar', data: malfCountData, barGap: '0%' },
        { name: '装备总数', type: 'bar', data: countData, barGap: '0%' },
      ],
      grid: {
        bottom: 50,
      },
...
const updateSearchParamsDay = () => {
  searchParams.value.day = parseInt(inputValue.value) || 50;
};
...
const handleQuery = async () => {
  ...
  const response = await malfregioncount(params);
