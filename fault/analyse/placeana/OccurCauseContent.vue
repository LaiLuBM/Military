...
import { malfregionlist, malfregionsubreason } from '@/api/fault/placeanalyse';
...
const renderChart = async (data: any) => {
  ...
  // Handle single value or comma-separated reasons
  ...
  const counts = malfRatioChart.equipMalfCount.includes(',')
    ? malfRatioChart.equipMalfCount.split(',').map((val: string) => {
      ...
      return isNaN(num) || num < 0 || num > 1000 ? 0 : num; // Sanitize
    })
  ...
  // Calculate y-axis max robustly
  const allCounts = lineSeries.flatMap((series: any) => series.data).filter((num: number) => num > 0 && num < 1000);
  const yAxisMax = allCounts.length ? Math.max(...allCounts) * 1.2 : 10;
...
const updateSearchParamsDay = () => {
  ...
  searchParams.value.day = numValue || 90;
};
