...
import { partfailurestats } from '@/api/fault/timeanalyse';
...
const renderChart = async (data: any) => {
  ...
  const pieData = data.stats[equipmentModel]?.pieData || {};
  const lineData = data.stats[equipmentModel]?.lineData || {};
  const pieTitle = data.description[equipmentModel]?.pietitle || '';
  ...
  const pieChartData = Object.keys(pieData).map((key) => ({
    name: key,
    value: pieData[key] * 100, // Convert to percentage
  }));
  ...
  const parts = Object.keys(lineData);
  ...
  // Generate colors for all parts
  const partColors: Record<string, string> = {};
  parts.forEach(part => {
    partColors[part] = getRandomColor();
  });

  const lineSeries = parts.map((part) => {
    const values = allYears.map((year) => {
      const value = lineData[part][year] || 0;
      return value;
    });
    return {
      name: part,
      ...
      symbolSize: 6,
      itemStyle: {
        color: partColors[part], // Use the generated color
      },
    };
  });
    ...
    series: [
      {
        ...
        data: pieChartData.map(item => ({
          ...item,
          itemStyle: {
            color: partColors[item.name] || '#999' // Use the same color as line chart
          }
        })),
        emphasis: {
          itemStyle: {
  ...
  try {
    ...
    chartDescription.value = description || '显示各部件在不同列装时间下的故障数及占比';
  } catch (error) {
...
const handleQuery = async () => {
  ...
  try {
    const response = await partfailurestats(params);
