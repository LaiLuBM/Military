...
import { regularityrecords } from '@/api/fault/timeanalyse';
...
const renderChart = (data: any) => {
  ...
  const { faultByYear, faultRatePerTenThousand } = data.summary;
  const description = data.description?.description || '';
  const title = data.description?.title || '';

  // Get all unique years from faultByYear and faultRatePerTenThousand, sorted numerically
  const years = [...new Set([...Object.keys(faultByYear), ...Object.keys(faultRatePerTenThousand)])]
  ...
  // Prepare data for fault count and fault rate
  const faultCountData = years.map(year => faultByYear[year] || 0);
  const faultRateData = years.map(year => faultRatePerTenThousand[year] || 0);
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        params.forEach((item: any) => {
          result += `${item.seriesName}: ${item.value}${item.seriesName === '故障数量' ? '台' : ''}<br>`;
        });
        return result;
      },
    },
...
const handleQuery = async () => {
  ...
  const response = await regularityrecords(params);
