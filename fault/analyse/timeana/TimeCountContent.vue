...
import { predictiontime } from '@/api/fault/timeanalyse';
...
const renderChart = (data: any) => {
  ...
  const modelNames = [...new Set(
    Object.values(faultData)
      .flat()
      .map((item: any) => item.modelName)
  )];
  ...
  const series = modelNames.map((modelName) => {
    const data = years.map((year) => {
      const yearData = faultData[year] || [];
      const modelData = yearData.find((item: any) => item.modelName === modelName);
      return modelData ? modelData.faultByYear : 0;
    });
    return {
      name: modelName,
      type: 'bar',
      data,
      barWidth: `${80 / modelNames.length}%`, // Dynamic bar width based on number of models
      itemStyle: {
  ...
  chart.setOption({
    ...
    yAxis: {
      ...
      name: '未来一年内预测故障数量 (台)',
      min: 0,
      max: (value: any) => Math.max(2, value.max), // Ensure visibility for small values
      interval: 1, // Force integer ticks
    },
    series,
    barGap: '10%', // Gap between bars of same category
    barCategoryGap: '20%', // Gap between different categories (years)
  });
...
const handleQuery = async () => {
  ...
  const response = await predictiontime(params);
