...
import { subsysfailurestats } from '@/api/fault/timeanalyse';
...
const renderChart = async (data: any) => {
  ...
  const equipmentModel = Object.keys(data.stats)[0];
  const radarData = data.stats[equipmentModel]?.radarData || {};
  const lineData = data.stats[equipmentModel]?.lineData || {};
  const radarTitle = data.description[equipmentModel]?.radartitle || '';
  const lineTitle = data.description[equipmentModel]?.linetitle || '';
  const description = data.description[equipmentModel]?.description || '';

  // Prepare radar chart data (left chart)
  const radarIndicators = Object.keys(radarData).map((key) => ({
    name: key,
    max: 1,
  }));
  const radarValues = Object.values(radarData) as number[];

  if (!radarIndicators.length || !radarValues.length) {
    leftChart.setOption({
      title: {
        text: '',
        left: 'center',
  ...
  const allYears = [...new Set(
    Object.values(lineData)
      .flatMap((system: any) => Object.keys(system))
  ...
  const systems = Object.keys(lineData);
  ...
  const lineSeries = systems.map((system) => {
    const values = allYears.map((year) => {
      const value = lineData[system][year] || 0;
      return value;
    });
    return {
      name: system,
      ...
      symbolSize: 6,
      itemStyle: {
        color: getRandomColor(), // Assign random color to each system
      },
    };
  });

  // Radar Chart (Left) - Keep original color
  const radarOption = {
    color: ['#5470C6'], // Default blue color for radar chart
    title: {
      text: radarTitle,
      left: 'center',
      top: 10,
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        params.value.forEach((value: number, index: number) => {
          result += `${radarIndicators[index].name}: ${(value * 100).toFixed(1)}%<br>`;
        });
        return result;
      },
    },
    ...
    series: [
      {
        name: equipmentModel,
        type: 'radar',
        data: [{ value: radarValues, name: equipmentModel }],
  ...
  // Line Chart (Right) - With random colors
  const lineOption = {
    title: {
      text: lineTitle,
      left: 'center',
      ...
      formatter: (params: any) => {
        ...
        params.forEach((item: any) => {
          if (item.value !== null && item.value !== undefined) {
            result += `${item.seriesName}: ${item.value.toFixed(0)}/台<br>`;
          }
        });
        return result;
      },
...
const handleQuery = async () => {
  ...
  try {
    const response = await subsysfailurestats(params);
